...
namespace AIAudioLanguageConverter.NPCKEditor
{
    ...
    public class NPCKHeader
    {
        ...
        public NPCKHeader(BinaryReader br,SupportedGames Mode,string fileName)
        {
            ...
            for(int i = 0; i < wemACount; i++)
            {
                ...
                //Console.WriteLine(languageEnum.ToString());
            ...
            var test = $"{br.BaseStream.Position:x}";
        ...
        public Wem GetWemFromId(uint id)
        {
            foreach (Wem wem in WemList)
            {
                if (wem.id == id)
        ...
        public int GetWemIndexFromId(uint id)
        {
            for (var i = 0; i < WemList.Count; i++)
            {
                if (WemList[i].id == id)
                    return i;
            }
